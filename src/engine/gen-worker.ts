...
ctx.onmessage = (e: MessageEvent) => {
  ...
  const res: GenResult = {
    ...
    data: chunk.data, heightmap: chunk.heightmap, torches, glowers,
    doors: [...sink.doorStates], torchFacings: [...sink.torchFacings], beds: [...sink.bedFacings],
    spawns,
  };
  ctx.postMessage({ type: 'done', res }, [chunk.data.buffer, chunk.heightmap.buffer, torches.buffer, glowers.buffer]);
};
