// Thrown items: warp pearls (teleport the thrower to where they land) and
// snowballs (a harmless knock-back, but they sting fiery Nether mobs).
...
export class Throwables {
  ...
  throw(id: number, x: number, y: number, z: number, dx: number, dy: number, dz: number): void {
    ...
    this.shots.push({ id, sprite, x, y, z, vx: dx * SPEED, vy: dy * SPEED + 1.5, vz: dz * SPEED, age: 0 });
  }
