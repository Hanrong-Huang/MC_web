...
- `verify-nether-wood.mjs` (`PORT`, `SHOT_DIR` env) — crimson/warped planks, slabs, stairs, fences, gates and both vines on the y=108 platform, icons + held models, climbing a vine, vines unravelling, bone meal, generated vines in both forests, and an old-format (u8 RLE) save loading + migrating. After Save & Quit, `#loading.hidden` is already set, so wait for a *new* `window.__game` before asserting on a reloaded world.
